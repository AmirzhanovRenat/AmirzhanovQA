...
SELECT ut.token  FROM  user_token ut join phpbb_users pu  on  ut.user_id  = pu.user_id  where pu.username  = 'Ольга' /*Токен юзера*/;

SELECT user_id  from phpbb_users pu where pu.username = 'Ольга';

SELECT * FROM purchase_payment_form ppf
LEFT JOIN money m ON m.user_id = ppf.user_id AND m.purchase_id = ppf.purchase_id/*СБП статусы конфирмед и таблица мани*/
